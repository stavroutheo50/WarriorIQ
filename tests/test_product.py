...
class NormalizedFighterBoxTests(unittest.TestCase):
    """Boxes drawn on the phone's 4K original must land on the 1280 upload."""

    def test_fractions_are_scaled_onto_the_uploaded_video(self):
        from app.main import StartPayload, _validated_fighter_box

        # What the browser sends: the same two fighters as fractions.
        payload = StartPayload(
            fighter_a_box=[0.30, 0.20, 0.45, 0.80],
            fighter_b_box=[0.55, 0.25, 0.70, 0.78],
            normalized_boxes=True,
        )
        # The file that actually arrived, re-encoded down from 3840x2160.
        width, height = 1280.0, 720.0
        scale = (width, height, width, height)
        pixels_a = [v * axis for v, axis in zip(payload.fighter_a_box, scale)]
        pixels_b = [v * axis for v, axis in zip(payload.fighter_b_box, scale)]

        box_a = _validated_fighter_box(pixels_a, width, height, "Fighter A")
        box_b = _validated_fighter_box(pixels_b, width, height, "Fighter B")
        self.assertEqual([round(v) for v in box_a], [384, 144, 576, 576])
        self.assertEqual([round(v) for v in box_b], [704, 180, 896, 562])

        # The same fractions on the 4K original describe the same fighters,
        # which is the whole point: pixels would not have survived the rescale.
        big_w, big_h = 3840.0, 2160.0
        big_scale = (big_w, big_h, big_w, big_h)
        big_a = _validated_fighter_box(
            [v * axis for v, axis in zip(payload.fighter_a_box, big_scale)],
            big_w, big_h, "Fighter A")
        self.assertAlmostEqual(big_a[0] / big_w, box_a[0] / width, places=6)
        self.assertAlmostEqual(big_a[3] / big_h, box_a[3] / height, places=6)

    def test_pixel_boxes_still_work_for_the_server_rendered_picker(self):
        """/select still posts pixels; the flag must default off."""
        from app.main import StartPayload

        payload = StartPayload(fighter_a_box=[10, 10, 90, 200], fighter_b_box=[120, 12, 200, 205])
        self.assertFalse(payload.normalized_boxes)
