...
LEGAL_DOCUMENTS = {
    ...
    "cookies": {
        ...
        "sections": [
            ...
            ("Advertising", "The same Google tag carries a Google Ads tag, which receives a page-view signal. Advertising storage, ad user data and ad personalisation are set to denied and are never granted by WarriorIQ, including under Accept All, so no advertising cookie is written and the signal is sent without personalisation. The optional marketing storage preference is recorded against your choice but does not currently enable advertising storage. Fight footage and report contents are not sent to Google. The operator must document the contracted Google entity, processing location, data-retention setting and transfer mechanism before production use."),
